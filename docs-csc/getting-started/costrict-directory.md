...
# Explore the .costrict directory

> Where CSC reads AGENTS.md, settings.json, hooks, Skills, Subagents, commands, rules, and auto memory. Explore the .costrict directory in your project and ~/.costrict in your home directory.

CSC reads instructions, settings, Skills, Subagents, and memory from your project directory and from `~/.costrict` in your home directory. Commit project files to git to share with your team; files in `~/.costrict` are personal configuration that applies to all your projects.

If you set `CLAUDE_CONFIG_DIR`, every `~/.costrict` path on this page will be under that directory instead.

Most users only need to edit `AGENTS.md` and `settings.json`. The rest of the directory is optional: add Skills, rules, or Subagents as needed.
...
- AGENTS.md loads into every session. If something only matters for specific tasks, move it to a Skill or a path-scoped rule so it loads only when needed
- List the commands you run most, like build, test, and format, so CSC knows them without you spelling them out each time
- Run `/memory` to open and edit AGENTS.md from within a session
- Also works at `.costrict/AGENTS.md` if you prefer to keep the project root clean
...
**Description:** Configures Model Context Protocol (MCP) servers that give CSC access to external tools: databases, APIs, browsers, and more. This file holds the project-scoped servers your whole team uses. Personal servers you want to keep to yourself go in `~/.costrict.json` instead.

**Tips:**

- Use environment variable references for secrets: `${GITHUB_TOKEN}`
- Lives at the project root, not inside `.costrict/`
- For servers only you need, run `csc mcp add --scope user`. This writes to `~/.costrict.json` instead of `.mcp.json`
...
**When loaded:** Overrides global `~/.costrict/settings.json`. Local settings, CLI flags, and managed settings override this

**Description:** Settings that CSC applies directly. Permissions control which commands and tools CSC can use; hooks run your scripts at specific points in a session. Unlike AGENTS.md, which CSC reads as guidance, these are enforced whether CSC follows them or not.
...
**Description:** Project instructions split into topic files that can load conditionally based on file paths. A rule without `paths:` frontmatter loads at session start like AGENTS.md; a rule with `paths:` loads only when CSC reads a matching file.

Like AGENTS.md, rules are guidance CSC reads, not configuration CSC enforces. For guaranteed behavior, use hooks or permissions.

**Tips:**
- Use `paths:` frontmatter with globs to scope rules to directories or file types
- Subdirectories work: `.costrict/rules/frontend/react.md` is discovered automatically
- When AGENTS.md approaches 200 lines, start splitting into rules
...
**Description:** Output styles are usually personal, so most live in `~/.costrict/output-styles/`. Put one here if your team shares a style, like a review mode everyone uses. See the Global section for the full explanation and example.
...
**Description:** Subagents with `memory: project` in their frontmatter get a dedicated memory directory here. This is distinct from your main session auto memory at `~/.costrict/projects/`: each Subagent reads and writes its own MEMORY.md, not yours.

**Tips:**
- Only created for Subagents that set the `memory:` frontmatter field
- This directory holds project-scoped Subagent memory, meant to be shared with your team. To keep memory out of version control use `memory: local`, which writes to `.costrict/agent-memory-local/` instead. For cross-project memory use `memory: user`, which writes to `~/.costrict/agent-memory/`
- The main session auto memory is a different feature; see `~/.costrict/projects/` in the Global section
...
- The `projects` key tracks per-project state like trust-dialog acceptance and last-session metrics. Permission rules you approve in-session go to `.costrict/settings.local.json` instead
...
## Global ~/.costrict/ directory

Your personal configuration across all projects. The global counterpart to your project .costrict/ directory. Files here apply to every project you work in and are never committed to any repository.
...
**Description:** Your global instruction file. Loaded alongside the project AGENTS.md at session start, so both are in context together. When instructions conflict, project-level instructions take priority. Keep this to preferences that apply everywhere: response style, commit format, personal conventions.

**Tips:**
- Keep it short since it loads into context for every project, alongside that project's own AGENTS.md
...
Settings follow a precedence order: project `settings.json` overrides any matching keys you set here. This is different from AGENTS.md, where global and project files are both loaded into context rather than merged key by key.
...
**Description:** Same as project .costrict/rules/ but applies everywhere. Use this for conventions you want across all your work, like personal code style or commit message format.
...
**Description:** Subagents with `memory: user` in their frontmatter store knowledge here that persists across all projects. For project-scoped Subagent memory, see `.costrict/agent-memory/` instead.
...
| `AGENTS.local.md` | Project root | Your private preferences for this project, loaded alongside AGENTS.md. Create manually and add to `.gitignore` |
| Installed Plugins | `~/.costrict/plugins/` | Cloned marketplaces, installed Plugin versions, and per-Plugin data, managed by the `csc plugin` command. Orphaned versions are deleted 7 days after a Plugin is updated or uninstalled. See Plugin cache |

`~/.costrict` also holds data CSC writes as it works: transcripts, prompt history, file snapshots, caches, and logs. See Application data below.

## File reference

This table lists every file the page covers. Project-scoped files live under your repo's `.costrict/` (or at the root for `AGENTS.md`, `.mcp.json`, and `.worktreeinclude`). Global-scoped files live in `~/.costrict/` and apply to all projects.
...
| --- | --- | --- | --- |
| `AGENTS.md` | Project and global | ✓ | Instructions loaded every session |
...
| `~/.costrict.json` | Global only | | App state, OAuth, UI toggles, personal MCP servers |
...
| `/memory` | Which AGENTS.md and rules files are loaded, plus auto memory entries |
...
Beyond the configuration you write, `~/.costrict` also holds data CSC writes during sessions. These files are plain text. Anything that passes through a tool gets written to transcripts on disk: file contents, command output, pasted text.
...
| `backups/` | Timestamped copies of `~/.costrict.json` taken before config migrations |
...
| Delete | You lose |
| --- | --- |
| `~/.costrict/projects/` | Resume, continue, and rollback of past sessions |
| `~/.costrict/history.jsonl` | Up-arrow prompt recall |
| `~/.costrict/file-history/` | Checkpoint recovery for past sessions |
| `~/.costrict/stats-cache.json` | Historical totals shown by `/cost` |
| `~/.costrict/backups/` | Rollback copies of `~/.costrict.json` from past config migrations |
| `~/.costrict/debug/`, `~/.costrict/plans/`, `~/.costrict/paste-cache/`, `~/.costrict/image-cache/`, `~/.costrict/session-env/` | No user-visible content |
| `~/.costrict/todos/` | None. Legacy directory that current versions no longer write to. |

Do not delete `~/.costrict.json`, `~/.costrict/settings.json`, or `~/.costrict/plugins/`: they hold your authentication, preferences, and installed Plugins.
