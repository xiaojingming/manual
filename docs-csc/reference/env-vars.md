...
| `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD` | Set to `1` to load AGENTS.md files from directories specified with `--add-dir`. By default, additional directories do not load memory files |
...
| `CLAUDE_CODE_DISABLE_CLAUDE_MDS` | Set to `1` to prevent loading any AGENTS.md memory files into context, including user, project, and auto-memory files |
...
| `CLAUDE_CODE_NEW_INIT` | Set to `1` to make `/init` run an interactive setup flow. The flow asks which files to generate, including AGENTS.md, skills, and hooks, before exploring the codebase and writing them. Without this variable, `/init` generates a AGENTS.md automatically without prompting |
...
| `CLAUDE_CODE_PLUGIN_CACHE_DIR` | Override the plugins root directory. Despite the name, this sets the parent directory, not the cache itself: marketplaces and the plugin cache live in subdirectories under this path. Defaults to `~/.costrict/plugins` |
...
| `CLAUDE_CODE_SIMPLE` | Set to `1` to run with a minimal system prompt and only the Bash, file read, and file edit tools. MCP tools from `--mcp-config` are still available. Disables auto-discovery of hooks, skills, plugins, MCP servers, auto memory, and AGENTS.md. The `--bare` CLI flag sets this |
