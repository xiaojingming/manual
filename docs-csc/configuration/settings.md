...
> Configure CSC with global and project-level settings, and environment variables.

CSC offers a variety of settings to configure its behavior to meet your needs. You can configure CSC by running the `/config` command when using the interactive REPL, which opens a tabbed Settings interface where you can view status information and modify configuration options.

## Configuration scopes

CSC uses a **scope system** to determine where configurations apply and who they're shared with. Understanding scopes helps you decide how to configure CSC for personal use, team collaboration, or enterprise deployment.
...
| **User**    | `~/.costrict/` directory                                                             | You, across all projects             | No                     |
| **Project** | `.costrict/` in repository                                                           | All collaborators on this repository | Yes (committed to git) |
| **Local**   | `.costrict/settings.local.json`                                                      | You, in this repository only         | No (gitignored)        |
...
Scopes apply to many CSC features:

| Feature         | User location             | Project location                   | Local location                 |
| :-------------- | :------------------------ | :--------------------------------- | :----------------------------- |
| **Settings**    | `~/.costrict/settings.json` | `.costrict/settings.json`            | `.costrict/settings.local.json`  |
| **Subagents**   | `~/.costrict/agents/`       | `.costrict/agents/`                  | None                           |
| **MCP servers** | `~/.costrict.json`          | `.mcp.json`                        | `~/.costrict.json` (per-project) |
| **Plugins**     | `~/.costrict/settings.json` | `.costrict/settings.json`            | `.costrict/settings.local.json`  |
| **AGENTS.md**   | `~/.costrict/AGENTS.md`     | `AGENTS.md` or `.costrict/AGENTS.md` | `AGENTS.local.md`              |

***

## Settings files

The `settings.json` file is the official mechanism for configuring CSC through hierarchical settings:

* **User settings** are defined in `~/.costrict/settings.json` and apply to all projects.
* **Project settings** are saved in your project directory:
  * `.costrict/settings.json` for settings that are checked into source control and shared with your team
  * `.costrict/settings.local.json` for settings that are not checked in, useful for personal preferences and experimentation. CSC will configure git to ignore `.costrict/settings.local.json` when it is created.
* **Managed settings**: For organizations that need centralized control, CSC supports multiple delivery mechanisms for managed settings. All use the same JSON format and cannot be overridden by user or project settings:

  * **Server-managed settings**: delivered from CoStrict's servers via the costrict.ai admin console. See server-managed settings.
  * **MDM/OS-level policies**: delivered through native device management on macOS and Windows:
    ...
    * Windows: `HKLM\SOFTWARE\Policies\CoStrict` registry key with a `Settings` value (REG\_SZ or REG\_EXPAND\_SZ) containing JSON (deployed via Group Policy or Intune)
    * Windows (user-level): `HKCU\SOFTWARE\Policies\CoStrict` (lowest policy priority, only used when no admin-level source exists)
  * **File-based**: `managed-settings.json` and `managed-mcp.json` deployed to system directories:

    * macOS: `/Library/Application Support/CoStrict/`
    * Linux and WSL: `/etc/claude-code/`
    * Windows: `C:\Program Files\CoStrict\`

    > **⚠️ Warning:** The legacy Windows path `C:\ProgramData\CoStrict\managed-settings.json` is no longer supported as of v2.1.75. Administrators who deployed settings to that location must migrate files to `C:\Program Files\CoStrict\managed-settings.json`.
  ...
  > **Note:** Managed deployments can also restrict **plugin marketplace additions** using `strictKnownMarketplaces`. For more information, see managed marketplace restrictions.

* **Other configuration** is stored in `~/.costrict.json`. This file contains your preferences (theme, notification settings, editor mode), OAuth session, user and local scope MCP server configurations, per-project state (allowed tools, trust settings), and various caches. Project-scoped MCP servers are stored separately in `.mcp.json`.

> **Note:** CSC automatically creates timestamped backups of configuration files and retains the five most recent backups to prevent data loss.

```json
{
  "$schema": "https://json.schemastore.org/csc-settings.json",
  "permissions": {
    "allow": [
      "Bash(npm run lint)",
      "Bash(npm run test *)",
...
The `$schema` line in the example above points to the official JSON schema for CSC settings. Adding it to your `settings.json` enables autocomplete and inline validation in VS Code, Cursor, and any other editor that supports JSON schema validation.
...
| `allowedChannelPlugins`           | (Managed settings only) Allowlist of channel plugins that may push messages. Replaces the default CoStrict allowlist when set. Undefined = fall back to the default, empty array = block all channel plugins. Requires `channelsEnabled: true`. See restrict which channel plugins can run                                                                                                                                                                                                       | `[{ "marketplace": "claude-plugins-official", "plugin": "telegram" }]`                                                         |
...
| `attribution`                     | Customize attribution for git commits and pull requests. See attribution settings                                                                                                                                                                                                                                                                                                                                                                                                                                               | `{"commit": "🤖 Generated with CSC", "pr": ""}`                                                                               |
| `autoMemoryDirectory`             | Custom directory for auto memory storage. Accepts `~/`-expanded paths. Not accepted in project settings (`.costrict/settings.json`) to prevent shared repos from redirecting memory writes to sensitive locations. Accepted from policy, local, and user settings                                                                                                                                                                                                                                                           | `"~/my-memory-dir"`                                                                                                            |
...
| `disableDeepLinkRegistration`     | Set to `"disable"` to prevent CSC from registering the `csc-cli://` protocol handler with the operating system on startup. Deep links let external tools open a CSC session with a pre-filled prompt via `csc-cli://open?q=...`. The `q` parameter supports multi-line prompts using URL-encoded newlines (`%0A`). Useful in environments where protocol handler registration is restricted or managed separately                                                                                                                  | `"disable"`                                                                                                                    |
...
| `fileSuggestion`                  | Configure a custom script for `@` file autocomplete. See file suggestion settings                                                                                                                                                                                                                                                                                                                                                                                                                                           | `{"type": "command", "command": "~/.costrict/file-suggestion.sh"}`                                                               |
...
| `includeCoAuthoredBy`             | **Deprecated**: Use `attribution` instead. Whether to include the `co-authored-by CSC` byline in git commits and pull requests (default: `true`)                                                                                                                                                                                                                                                                                                                                                                                                      | `false`                                                                                                                        |
| `includeGitInstructions`          | Include built-in commit and PR workflow instructions and the git status snapshot in CSC's system prompt (default: `true`). Set to `false` to remove both, for example when using your own git workflow skills. The `CLAUDE_CODE_DISABLE_GIT_INSTRUCTIONS` environment variable takes precedence over this setting when set                                                                                                                                                                                                                            | `false`                                                                                                                        |
| `language`                        | Configure CSC's preferred response language (e.g., `"japanese"`, `"spanish"`, `"french"`). CSC will respond in this language by default. Also sets the voice dictation language                                                                                                                                                                                                                                                                                                           | `"japanese"`                                                                                                                   |
| `model`                           | Override the default model to use for CSC                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `"claude-sonnet-4-6"`                                                                                                          |
| `modelOverrides`                  | Map CoStrict model IDs to provider-specific model IDs such as Bedrock inference profile ARNs. Each model picker entry uses its mapped value when calling the provider API. See override model IDs per version                                                                                                                                                                                                                                                                                        | `{"claude-opus-4-6": "arn:aws:bedrock:..."}`                                                                                   |
...
| `plansDirectory`                  | Customize where plan files are stored. Path is relative to project root. Default: `~/.costrict/plans`                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `"./plans"`                                                                                                                    |
...
| `spinnerTipsEnabled`              | Show tips in the spinner while CSC is working. Set to `false` to disable tips (default: `true`)                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `false`                                                                                                                        |
...
| `statusLine`                      | Configure a custom status line to display context. See `statusLine` documentation                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `{"type": "command", "command": "~/.costrict/statusline.sh"}`                                                                    |
...
These settings are stored in `~/.costrict.json` rather than `settings.json`. Adding them to `settings.json` will trigger a schema validation error.

| Key                          | Description                                                                                                                                                                                                                                                                                                          | Example        |
| :--------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------- |
| `autoConnectIde`             | Automatically connect to a running IDE when CSC starts from an external terminal. Default: `false`. Appears in `/config` as **Auto-connect to IDE (external terminal)** when running outside a VS Code or JetBrains terminal                                                                                 | `true`         |
| `autoInstallIdeExtension`    | Automatically install the CSC IDE extension when running from a VS Code terminal. Default: `true`. Appears in `/config` as **Auto-install IDE extension** when running inside a VS Code or JetBrains terminal. You can also set the `CLAUDE_CODE_IDE_SKIP_AUTO_INSTALL` environment variable | `false`        |
...
| `deny`                              | Array of permission rules to deny tool use. Use this to exclude sensitive files from CSC access. See permission rule syntax and Bash permission limitations                                       | `[ "WebFetch", "Bash(curl *)", "Read(./.env)", "Read(./secrets/**)" ]` |
| `additionalDirectories`             | Additional working directories for file access. Most `.costrict/` configuration is not discovered from these directories                                            | `[ "../docs/" ]`                                                       |
| `defaultMode`                       | Default permission mode when opening CSC. Valid values: `default`, `acceptEdits`, `plan`, `auto`, `dontAsk`, `bypassPermissions`. The `--permission-mode` CLI flag overrides this setting for a single session                                         | `"acceptEdits"`                                                        |
...
| `filesystem.allowWrite`                | Additional paths where sandboxed commands can write. Arrays are merged across all settings scopes: user, project, and managed paths are combined, not replaced. Also merged with paths from `Edit(...)` allow permission rules. See sandbox path prefixes below.                                                              | `["/tmp/build", "~/.kube"]`     |
...
| `network.httpProxyPort`                | HTTP proxy port used if you wish to bring your own proxy. If not specified, CSC will run its own proxy.                                                                                                                                                                                                                                      | `8080`                          |
| `network.socksProxyPort`               | SOCKS5 proxy port used if you wish to bring your own proxy. If not specified, CSC will run its own proxy.                                                                                                                                                                                                                                    | `8081`                          |
...
* **`sandbox.filesystem` settings** (shown above): Control paths at the OS-level sandbox boundary. These restrictions apply to all subprocess commands (e.g., `kubectl`, `terraform`, `npm`), not just CSC's file tools.
* **Permission rules**: Use `Edit` allow/deny rules to control CSC's file tool access, `Read` deny rules to block reads, and `WebFetch` allow/deny rules to control network domains. Paths from these rules are also merged into the sandbox configuration.

### Attribution settings

CSC adds attribution to git commits and pull requests. These are configured separately:
...
```json
{
  "fileSuggestion": {
    "type": "command",
    "command": "~/.costrict/file-suggestion.sh"
  }
}
```
...
This hierarchy ensures that organizational policies are always enforced while still allowing teams and individuals to customize their experience. The same precedence applies whether you run CSC from the CLI, the VS Code extension, or a JetBrains IDE.
...
Run `/status` inside CSC to see which settings sources are active and where they come from. The output shows each configuration layer (managed, user, project) along with its origin, such as `Enterprise managed settings (remote)`, `Enterprise managed settings (plist)`, `Enterprise managed settings (HKLM)`, or `Enterprise managed settings (file)`. If a settings file contains errors, `/status` reports the issue so you can fix it.

### Key points about the configuration system

* **Memory files (`AGENTS.md`)**: Contain instructions and context that CSC loads at startup
* **Settings files (JSON)**: Configure permissions, environment variables, and tool behavior
* **Skills**: Custom prompts that can be invoked with `/skill-name` or loaded by CSC automatically
* **MCP servers**: Extend CSC with additional tools and integrations
* **Precedence**: Higher-level configurations (Managed) override lower-level ones (User/Project)
* **Inheritance**: Settings are merged, with more specific settings adding to or overriding broader ones

### System prompt

CSC's internal system prompt is not published. To add custom instructions, use `AGENTS.md` files or the `--append-system-prompt` flag.

### Excluding sensitive files

To prevent CSC from accessing files containing sensitive information like API keys, secrets, and environment files, use the `permissions.deny` setting in your `.costrict/settings.json` file:
...
CSC supports custom AI subagents that can be configured at both user and project levels. These subagents are stored as Markdown files with YAML frontmatter:

* **User subagents**: `~/.costrict/agents/` - Available across all your projects
* **Project subagents**: `.costrict/agents/` - Specific to your project and can be shared with your team
...
CSC supports a plugin system that lets you extend functionality with skills, agents, hooks, and MCP servers. Plugins are distributed through marketplaces and can be configured at both user and repository levels.
...
* **User settings** (`~/.costrict/settings.json`): Personal plugin preferences
* **Project settings** (`.costrict/settings.json`): Project-specific plugins shared with team
* **Local settings** (`.costrict/settings.local.json`): Per-machine overrides (not committed)
...
* **macOS**: `/Library/Application Support/CoStrict/managed-settings.json`
* **Linux and WSL**: `/etc/claude-code/managed-settings.json`
* **Windows**: `C:\Program Files\CoStrict\managed-settings.json`
...
Environment variables let you control CSC behavior without editing settings files. Any variable can also be configured in `settings.json` under the `env` key to apply it to every session or roll it out to your team.

See the environment variables reference for the full list.

## Tools available to CSC

CSC has access to a set of tools for reading, editing, searching, running commands, and orchestrating subagents. Tool names are the exact strings you use in permission rules and hook matchers.
