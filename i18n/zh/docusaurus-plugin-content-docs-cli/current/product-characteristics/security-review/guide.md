...
在编码阶段通过 CLI 进行交互式安全扫描，实时辅助开发人员发现并修复安全问题。
...
![CLI使用1-进入交互窗口](./img/CLI使用1-进入交互窗口.webp)
...
![指定审查范围](./img/CLI使用2-指定审查范围.webp)

### 步骤 3：查看扫描报告

触发安全审查后，CLI 交互窗口会实时展示扫描过程。扫描过程中如涉及危险操作，需要用户手动确认后方可继续。扫描时长与代码量有关，从几分钟到几十分钟不等。扫描完成后，会在项目本地生成安全审查报告。报告包含以下三种类型：
...
![CLI使用3-安全扫描报告](./img/CLI使用3-安全扫描报告.webp)
