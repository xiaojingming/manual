...
| `UserPromptSubmit`   | 当你提交提示时，在 CSC 处理它之前                                                                                                   |
...
| `InstructionsLoaded` | 当 AGENTS.md 或 `.costrict/rules/*.md` 文件被加载到上下文中时。在会话开始时以及文件在会话期间被延迟加载时触发         |
| `ConfigChange`       | 当配置文件在会话期间更改时                                                                                                     |
| `CwdChanged`         | 当工作目录更改时，例如当 CSC 执行 `cd` 命令时。适用于与 direnv 等工具的响应式环境管理 |
...
CSC 读取 JSON 决策，阻止工具调用，并向 CSC 显示原因。
...
| 位置                                                   | 作用范围                         | 可共享                          |
| :--------------------------------------------------------- | :---------------------------- | :--------------------------------- |
| `~/.costrict/settings.json`                                  | 你的所有项目             | 否，仅限本机          |
| `.costrict/settings.json`                                    | 单个项目                | 是，可以提交到仓库  |
...
* **提示 hooks**（`type: "prompt"`）：向 CSC 模型发送提示进行单轮评估。模型以 JSON 返回是/否决策。请参阅基于提示的 hooks。
...
此示例使用 `$CLAUDE_PROJECT_DIR` 在任何 `Write` 或 `Edit` 工具调用后从项目的 `.costrict/hooks/` 目录运行样式检查器：
...
* `User`：来自 `~/.costrict/settings.json`
* `Project`：来自 `.costrict/settings.json`
* `Local`：来自 `.costrict/settings.local.json`
* `Plugin`：来自插件的 `hooks/hooks.json`
* `Session`：在当前会话的内存中注册
* `Built-in`：由 CSC 内部注册

选择一个 hook 会打开一个详细视图，显示其事件、匹配器、类型、源文件以及完整的命令、提示或 URL。菜单是只读的：要添加、修改或移除 hooks，请直接编辑设置 JSON 或要求 CSC 进行更改。
...
**退出码 0** 表示成功。CSC 解析 stdout 以获取 JSON 输出字段。JSON 输出仅在退出码 0 时处理。对于大多数事件，stdout 写入调试日志但不显示在记录中。例外是 `UserPromptSubmit` 和 `SessionStart`，其中 stdout 作为 CSC 可以看到并据此行动的上下文添加。

**退出码 2** 表示阻塞错误。CSC 忽略 stdout 及其中的任何 JSON。相反，stderr 文本作为错误消息反馈给 CSC。效果取决于事件：`PreToolUse` 阻止工具调用，`UserPromptSubmit` 拒绝提示，等等。请参阅每个事件的退出码 2 行为以获取完整列表。
...
| `PostToolUse`        | 否         | 向 CSC 显示 stderr（工具已运行）                                                                                            |
| `PostToolUseFailure` | 否         | 向 CSC 显示 stderr（工具已失败）                                                                                         |
...
| 字段            | 默认值 | 描述                                                                                                                |
| :--------------- | :------ | :------------------------------------------------------------------------------------------------------------------------- |
| `continue`       | `true`  | 如果为 `false`，CSC 在 hook 运行后完全停止处理。优先于任何事件特定的决策字段 |
| `stopReason`     | 无    | 当 `continue` 为 `false` 时向用户显示的消息。不向 CSC 显示                                                  |
...
使用 `hookSpecificOutput` 进行更丰富的控制：允许、拒绝或升级给用户。你还可以在工具运行之前修改工具输入或为 CSC 注入额外的上下文。请参阅 PreToolUse 决策控制以获取完整的选项集。
...
当 CSC 开始新会话或恢复现有会话时运行。适用于加载开发上下文，如现有问题或代码库的最近更改，或设置环境变量。对于不需要脚本的静态上下文，请改用 AGENTS.md。
...
你的 hook 脚本打印到 stdout 的任何文本都会作为 CSC 的上下文添加。除了所有 hooks 可用的 JSON 输出字段外，你还可以返回这些事件特定的字段：

| 字段               | 描述                                                               |
| :------------------ | :------------------------------------------------------------------------ |
| `additionalContext` | 添加到 CSC 上下文的字符串。多个 hooks 的值会连接起来 |
...
当 `AGENTS.md` 或 `.costrict/rules/*.md` 文件被加载到上下文中时触发。此事件在会话开始时为急切加载的文件触发，稍后当文件被延迟加载时再次触发，例如当 CSC 访问包含嵌套 `AGENTS.md` 的子目录或具有 `paths:` frontmatter 的条件规则匹配时。该 hook 不支持阻止或决策控制。它出于可观察性目的异步运行。
...
当用户提交提示时运行，在 CSC 处理它之前。这允许你根据提示/对话添加额外的上下文、验证提示或阻止某些类型的提示。
...
| `additionalContext` | 添加到 CSC 上下文的字符串                                                                                   |
...
在 CSC 创建工具参数之后、处理工具调用之前运行。按工具名称匹配：`Bash`、`Edit`、`Write`、`Read`、`Glob`、`Grep`、`Agent`、`WebFetch`、`WebSearch`、`AskUserQuestion`、`ExitPlanMode` 以及任何 MCP 工具名称。
...
| `answers`   | object | `{"Which framework?": "React"}`                                                                                    | 可选。将问题文本映射到选定的选项标签。多选答案用逗号连接标签。CSC 不会设置此字段；通过 `updatedInput` 提供它以编程方式回答 |
...
| `permissionDecisionReason` | 对于 `"allow"` 和 `"ask"`，向用户显示但不向 CSC 显示。对于 `"deny"`，向 CSC 显示。对于 `"defer"`，忽略                                                                                                                                                           |
| `updatedInput`             | 在执行之前修改工具的输入参数。替换整个输入对象，因此请包含未更改的字段以及修改后的字段。与 `"allow"` 结合以自动批准，或与 `"ask"` 结合以向用户显示修改后的输入。对于 `"defer"`，忽略              |
| `additionalContext`        | 在工具执行之前添加到 CSC 上下文的字符串。对于 `"defer"`，忽略                                                                                                                                                                                            |
...
`"defer"` 适用于将 `csc -p` 作为子进程运行并读取其 JSON 输出的集成，例如 Agent SDK 应用程序或基于 CSC 构建的自定义 UI。它让调用进程可以在工具调用处暂停 CSC，通过自己的界面收集输入，并从停下的地方恢复。CSC 仅在使用 `-p` 标志的非交互模式下接受此值。在交互会话中，它会记录警告并忽略 hook 结果。

> **注意：**
> `defer` 值需要 CSC v2.1.89 或更高版本。早期版本无法识别它，工具会通过正常的权限流程继续。

`AskUserQuestion` 工具是典型情况：CSC 想问用户一些事情，但没有终端可以回答。往返过程如下：

1. CSC 调用 `AskUserQuestion`。`PreToolUse` hook 触发。
...
5. hook 返回 `permissionDecision: "allow"` 并在 `updatedInput` 中附带答案。工具执行，CSC 继续。

`deferred_tool_use` 字段携带工具的 `id`、`name` 和 `input`。`input` 是 CSC 为工具调用生成的参数，在执行之前捕获：
...
`"defer"` 仅在 CSC 在轮次中发出单个工具调用时有效。如果 CSC 同时发出多个工具调用，`"defer"` 会被忽略并显示警告，工具通过正常的权限流程继续。存在此约束是因为恢复只能重新运行一个工具：没有办法从一批调用中延迟一个而不让其他调用未解决。
...
| `message`            | 仅用于 `"deny"`：告诉 CSC 为什么权限被拒绝                                                                                                       |
| `interrupt`          | 仅用于 `"deny"`：如果为 `true`，停止 CSC                                                                                                                          |
...
`PostToolUse` hooks 可以在工具执行后向 CSC 提供反馈。除了所有 hooks 可用的 JSON 输出字段外，你的 hook 脚本可以返回这些事件特定的字段：

| 字段                  | 描述                                                                                |
| :--------------------- | :----------------------------------------------------------------------------------------- |
| `decision`             | `"block"` 以 `reason` 提示 CSC。省略以允许操作继续            |
| `reason`               | 当 `decision` 为 `"block"` 时向 CSC 显示的解释                                   |
| `additionalContext`    | 供 CSC 考虑的额外上下文                                                  |
...
当工具执行失败时运行。此事件在抛出错误或返回失败结果的工具调用时触发。使用它来记录失败、发送警报或向 CSC 提供纠正反馈。
...
`PostToolUseFailure` hooks 可以在工具失败后向 CSC 提供上下文。除了所有 hooks 可用的 JSON 输出字段外，你的 hook 脚本可以返回这些事件特定的字段：

| 字段               | 描述                                                   |
| :------------------ | :------------------------------------------------------------ |
| `additionalContext` | 供 CSC 与错误一起考虑的额外上下文 |
...
使用单独的匹配器根据通知类型运行不同的处理程序。此配置在 CSC 需要权限批准时触发特定于权限的警报脚本，在 CSC 空闲时触发不同的通知：
...
当通过 Agent 工具生成 CSC 子代理时运行。支持匹配器按代理类型名称过滤（内置代理如 `Bash`、`Explore`、`Plan`，或来自 `.costrict/agents/` 的自定义代理名称）。
...
除通用输入字段外，Stop hooks 接收 `stop_hook_active` 和 `last_assistant_message`。当 CSC 已经作为 stop hook 的结果继续时，`stop_hook_active` 字段为 `true`。检查此值或处理记录以防止 CSC 无限期运行。`last_assistant_message` 字段包含 CSC 最终响应的文本内容，因此 hooks 可以访问它而无需解析记录文件。
...
`Stop` 和 `SubagentStop` hooks 可以控制 CSC 是否继续。除了所有 hooks 可用的 JSON 输出字段外，你的 hook 脚本可以返回这些事件特定的字段：

| 字段      | 描述                                                                |
| :--------- | :------------------------------------------------------------------------- |
| `decision` | `"block"` 阻止 CSC 停止。省略以允许 CSC 停止      |
| `reason`   | 当 `decision` 为 `"block"` 时必需。告诉 CSC 为什么应该继续 |
...
当轮次因 API 错误结束时代替 Stop 运行。输出和退出码被忽略。使用此来在 CSC 由于速率限制、身份验证问题或其他 API 错误无法完成响应时记录失败、发送警报或采取恢复操作。
...
| `last_assistant_message` | 对话中显示的渲染错误文本。与 `Stop` 和 `SubagentStop` 中此字段保存 CSC 的对话输出不同，对于 `StopFailure`，它包含 API 错误字符串本身，如 `"API Error: Rate limit reached"` |
...
| 匹配器            | 触发时机                             |
| :----------------- | :---------------------------------------- |
| `user_settings`    | `~/.costrict/settings.json` 更改         |
| `project_settings` | `.costrict/settings.json` 更改           |
| `local_settings`   | `.costrict/settings.local.json` 更改     |
| `policy_settings`  | 托管策略设置更改            |
| `skills`           | `.costrict/skills/` 中的技能文件更改 |
...
当工作目录在会话期间更改时运行，例如当 CSC 执行 `cd` 命令时。使用此来响应目录更改：重新加载环境变量、激活项目特定的工具链或自动运行设置脚本。与 FileChanged 配合使用，用于管理每个目录环境的 direnv 等工具。
...
WorktreeCreate 的清理对应项。此 hook 在工作树被移除时触发，要么在你退出 `--worktree` 会话并选择移除它时，要么在具有 `isolation: "worktree"` 的子代理完成时。对于基于 git 的工作树，CSC 使用 `git worktree remove` 自动处理清理。如果你为非 git 版本控制系统配置了 WorktreeCreate hook，请配对一个 WorktreeRemove hook 来处理清理。没有的话，工作树目录将留在磁盘上。
...
1. 将 hook 输入和你的提示发送给 CSC 模型，默认为 Haiku
2. LLM 以包含决策的结构化 JSON 响应
3. CSC 自动处理决策

### 提示 Hook 配置

将 `type` 设置为 `"prompt"` 并提供 `prompt` 字符串而不是 `command`。使用 `$ARGUMENTS` 占位符将 hook 的 JSON 输入数据注入到你的提示文本中。CSC 将组合的提示和输入发送给快速的 CSC 模型，后者返回 JSON 决策。

此 `Stop` hook 要求 LLM 在允许 CSC 完成之前评估所有任务是否完成：
...
| `reason` | 当 `ok` 为 `false` 时必需。向 CSC 显示的解释 |

### 示例：多条件 Stop hook

此 `Stop` hook 使用详细的提示在允许 CSC 停止之前检查三个条件。如果 `"ok"` 为 `false`，CSC 继续工作，以提供的原因为下一步指令。`SubagentStop` hooks 使用相同的格式来评估子代理是否应该停止：
...
此 `Stop` hook 在允许 CSC 完成之前验证所有单元测试是否通过：
...
默认情况下，hooks 会阻塞 CSC 的执行直到完成。对于长时间运行的任务，如部署、测试套件或外部 API 调用，设置 `"async": true` 以在 CSC 继续工作的同时在后台运行 hook。异步 hooks 无法阻止或控制 CSC 的行为：响应字段如 `decision`、`permissionDecision` 和 `continue` 没有效果，因为它们本应控制的操作已经完成。

### 配置异步 Hook

在命令 hook 的配置中添加 `"async": true` 以在后台运行而不阻塞 CSC。此字段仅在 `type: "command"` hooks 上可用。

此 hook 在每次 `Write` 工具调用后运行测试脚本。CSC 立即继续工作，而 `run-tests.sh` 执行最多 120 秒。当脚本完成时，其输出在下一个对话轮次中传递：
...
后台进程退出后，如果 hook 生成了带有 `systemMessage` 或 `additionalContext` 字段的 JSON 响应，该内容将在下一个对话轮次中作为上下文传递给 CSC。

异步 hook 完成通知默认被抑制。要查看它们，请使用 `Ctrl+O` 启用详细模式或使用 `--verbose` 启动 CSC。

### 示例：文件更改后运行测试

此 hook 在 CSC 写入文件时在后台启动测试套件，然后在测试完成时将结果报告给 CSC。将此脚本保存到项目中的 `.costrict/hooks/run-tests-async.sh` 并使用 `chmod +x` 使其可执行：
...
然后将此配置添加到项目根目录的 `.costrict/settings.json` 中。`async: true` 标志让 CSC 在测试运行时继续工作：
...
Hook 执行详细信息，包括哪些 hooks 匹配、它们的退出码以及完整的 stdout 和 stderr，都写入调试日志文件。使用 `csc --debug-file <path>` 启动 CSC 以将日志写入已知位置，或运行 `csc --debug` 并在 `~/.costrict/debug/<session-id>.txt` 处读取日志。`--debug` 标志不会打印到终端。
