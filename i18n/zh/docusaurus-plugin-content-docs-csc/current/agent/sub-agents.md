...
每个Subagent在自己的上下文窗口中运行，具有自定义系统提示、特定工具访问权限和独立权限。当 CSC 遇到与Subagents描述匹配的任务时，它会委托给该Subagents，Subagents独立工作并返回结果。要实际查看上下文节省效果，上下文窗口可视化功能演示了一个Subagents在其独立窗口中处理研究的会话。
...
调用 Explore 时，CSC 会指定彻底程度：**quick** 用于定向查找，**medium** 用于平衡探索，**very thorough** 用于全面分析。
...
切换到 **Library** 标签页，选择 **Create new agent**，然后选择 **Personal**。这会将Subagents保存到 `~/.costrict/agents/`，使其在所有项目中可用。
...
选择 **User scope** 为Subagents提供位于 `~/.costrict/agent-memory/` 的持久记忆目录。Subagents使用它在对话之间积累见解，例如代码库模式和反复出现的问题。如果你不希望Subagents持久化学习内容，选择 **None**。
...
CSC 会委托给你的新Subagents，它会扫描代码库并返回改进建议。
...
**项目Subagents**（`.costrict/agents/`）非常适合特定于代码库的Subagents。将它们提交到版本控制，以便你的团队可以协作使用和改进它们。

项目Subagents通过从当前工作目录向上遍历来发现。使用 `--add-dir` 添加的目录仅授予文件访问权限，不会扫描Subagents。要跨项目共享Subagents，使用 `~/.costrict/agents/` 或Plugins。

**用户Subagents**（`~/.costrict/agents/`）是在你所有项目中可用的个人Subagents。
...
**托管Subagents**由组织管理员部署。将 markdown 文件放在托管设置目录内的 `.costrict/agents/` 中，使用与项目和用户Subagents相同的 frontmatter 格式。托管定义优先于同名项目和用户Subagents。

**PluginsSubagents**来自你安装的Plugins。它们与你的自定义Subagents一起出现在 `/agents` 中。有关创建PluginsSubagents的详细信息，请参阅Plugins组件参考。

> **注意：** 出于安全原因，PluginsSubagents不支持 `hooks`、`mcpServers` 或 `permissionMode` frontmatter 字段。从Plugins加载代理时，这些字段会被忽略。如果你需要它们，将代理文件复制到 `.costrict/agents/` 或 `~/.costrict/agents/`。你也可以在 `settings.json` 或 `settings.local.json` 中的 `permissions.allow` 中添加规则，但这些规则适用于整个会话，而不仅仅是PluginsSubagents。
...
| `description`      | 是   | CSC 何时应委托给此Subagents                                                                                                                                                                                                                 |
...
> **⚠️ 警告：** 谨慎使用 `bypassPermissions`。它会跳过权限提示，允许Subagents在未经批准的情况下执行操作。对 `.git`、`.claude`、`.vscode`、`.idea` 和 `.husky` 目录的写入仍会提示确认，但 `.costrict/commands`、`.costrict/agents` 和 `.costrict/skills` 除外。有关详细信息，请参阅权限模式。
...
| 作用域    | 位置                                           | 使用场景                                                 |
| :-------- | :--------------------------------------------- | :------------------------------------------------------- |
| `user`    | `~/.costrict/agent-memory/<name-of-agent>/`      | Subagents应在所有项目中记住学习内容                          |
| `project` | `.costrict/agent-memory/<name-of-agent>/`        | Subagents的知识是项目特定的，可通过版本控制共享              |
| `local`   | `.costrict/agent-memory-local/<name-of-agent>/`  | Subagents的知识是项目特定的，但不应提交到版本控制            |
...
你可以通过将特定Subagents添加到设置中的 `deny` 数组来阻止 CSC 使用它们。使用格式 `Agent(subagent-name)`，其中 `subagent-name` 匹配Subagents的 name 字段。
...
对于自然语言，没有特殊语法。命名Subagents，CSC 通常会委托：
...
**@-提及Subagents。** 输入 `@` 并从自动完成中选择Subagents，就像你 @-提及文件一样。这确保特定Subagents运行，而不是将选择留给 CSC：

```text
@"code-reviewer (agent)" look at the auth changes
```

你的完整消息仍然发送给 CSC，CSC 根据你的请求编写Subagents的任务提示。@-提及控制 CSC 调用哪个Subagents，而不是它接收什么提示。
...
Subagents的系统提示完全替换默认的 CSC 系统提示，与 `--system-prompt` 的方式相同。`AGENTS.md` 文件和项目记忆仍然通过正常的消息流加载。代理名称在启动头中显示为 `@<name>`，以便你确认它处于活动状态。

这适用于内置和自定义Subagents，当你恢复会话时选择会持久化。

对于Plugins提供的Subagents，传递限定名称：`csc --agent <plugin-name>:<agent-name>`。

要使其成为项目中每个会话的默认值，在 `.costrict/settings.json` 中设置 `agent`：
...
对于多步骤工作流，让 CSC 按顺序使用Subagents。每个Subagents完成其任务并将结果返回给 CSC ，然后 CSC 将相关上下文传递给下一个Subagents。
...
如果你想要显式引用它，你也可以向 CSC 请求代理 ID，或在 `~/.costrict/projects/{project}/{sessionId}/subagents/` 的记录文件中找到 ID。每条记录存储为 `agent-{agentId}.jsonl`。
...
Hooks通过 stdin 接收 JSON，其中 Bash 命令在 `tool_input.command` 中。退出码 2 阻止操作并将错误消息反馈给 CSC。有关退出码的详细信息，请参阅Hooks，有关完整的输入模式，请参阅Hooks输入。
