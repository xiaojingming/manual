...
打开 `~/.costrict/settings.json` 并添加一个 `Notification` Hooks。以下示例使用 `osascript`（适用于 macOS）；有关 Linux 和 Windows 命令，请参阅当 Claude 需要输入时获取通知。
...
此Hooks使用 `Notification` 事件，当 CSC 等待输入或权限时触发。以下各节使用平台的原生通知命令。将此添加到 `~/.costrict/settings.json`：
...
此Hooks使用 `PostToolUse` 事件和 `Edit|Write` 匹配器，因此它只在文件编辑工具之后运行。命令使用 `jq` 提取编辑的文件路径并将其传递给 Prettier。将此添加到项目根目录的 `.costrict/settings.json` 中：
...
将此保存到 `.costrict/hooks/protect-files.sh`：
...
在 `.costrict/settings.json` 中添加一个 `PreToolUse` Hooks，在任何 `Edit` 或 `Write` 工具调用之前运行脚本：
...
你的命令写入 stdout 的任何文本都会添加到 CSC 的上下文中。此示例提醒 CSC 项目约定和最近的工作。将此添加到项目根目录的 `.costrict/settings.json` 中：
...
你可以将 `echo` 替换为任何产生动态输出的命令，例如 `git log --oneline -5` 来显示最近的提交。对于在每次会话开始时注入上下文，考虑使用 AGENTS.md。对于环境变量，请参阅参考中的 `CLAUDE_ENV_FILE`。

### 审计配置变更

跟踪会话期间设置或Skills文件何时更改。当外部进程或编辑器修改配置文件时，`ConfigChange` 事件会触发，因此你可以记录变更以进行合规性检查或阻止未经授权的修改。

此示例将每次变更追加到审计日志中。将此添加到 `~/.costrict/settings.json`：
...
`CwdChanged` Hooks解决了这个问题：它每次 CSC 更改目录时运行，因此你可以为新位置重新加载正确的变量。Hooks将更新的值写入 `CLAUDE_ENV_FILE`，CSC 在每个 Bash 命令之前应用它。将此添加到 `~/.costrict/settings.json`：
...
匹配器将Hooks范围限定为仅 `ExitPlanMode`，因此不会影响其他提示。将此添加到 `~/.costrict/settings.json`：
...
| `InstructionsLoaded` | 当 AGENTS.md 或 `.costrict/rules/*.md` 文件被加载到上下文中时。在会话开始时和会话期间延迟加载文件时触发         |
...
* 确认设置文件位于正确的位置：`.costrict/settings.json` 用于项目Hooks，`~/.costrict/settings.json` 用于全局Hooks
