...
> **⚠️ 警告：** `bypassPermissions` 模式会跳过权限提示。写入 `.git`、`.costrict`、`.vscode`、`.idea` 和 `.husky` 目录仍会提示确认，以防止意外损坏仓库状态、编辑器配置和 git 钩子。写入 `.costrict/commands`、`.costrict/agents` 和 `.costrict/skills` 豁免且不提示，因为 CSC 在创建技能、子代理和命令时会常规写入这些目录。仅在 CSC 无法造成损害的隔离环境（如容器或 VM）中使用此模式。管理员可以通过在托管设置中将 `permissions.disableBypassPermissionsMode` 设置为 `"disable"` 来阻止此模式。
...
> **⚠️ 警告：** 试图约束命令参数的 Bash 权限模式是脆弱的。例如，`Bash(curl http://github.com/ *)` 旨在将 curl 限制为 GitHub URL，但不会匹配以下变体：
  ...
  * 通过 AGENTS.md 告知 CSC 你允许的 curl 模式
...
* `Edit(/docs/**)`：编辑 `<project>/docs/` 中的文件（不是 `/docs/`，也不是 `<project>/.costrict/docs/`）
...
添加目录扩展了 CSC 可以读取和编辑文件的位置。它不会使该目录成为完整的配置根目录：大多数 `.costrict/` 配置不会从附加目录发现，但有少数类型作为例外加载。

以下配置类型从 `--add-dir` 目录加载：

| 配置                                      | 从 `--add-dir` 加载                                           |
| :------------------------------------------------- | :---------------------------------------------------------------- |
| `.costrict/skills/` 中的技能          | 是，支持实时重载                                             |
| `.costrict/settings.json` 中的插件设置         | 仅 `enabledPlugins` 和 `extraKnownMarketplaces`                |
| AGENTS.md 文件和 `.costrict/rules/` | 仅在设置 `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1` 时 |

其他所有内容，包括子代理、命令、输出样式、钩子和其他设置，仅从当前工作目录及其父目录、`~/.costrict/` 的用户目录和托管设置中发现。要跨项目共享该配置，使用以下方法之一：

* **用户级配置**：将文件放在 `~/.costrict/agents/`、`~/.costrict/output-styles/` 或 `~/.costrict/settings.json` 中，使其在每个项目中可用
* **插件**：将配置打包为插件，供团队安装
* **从配置目录启动**：从包含你想要的 `.costrict/` 配置的目录运行 CSC
...
| 设置                                        | 描述                                                                                                                                                                                                                                 |
| :--------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `allowedChannelPlugins`                        | 允许推送消息的通道插件白名单。设置后替换默认的 CoStrict 白名单。需要 `channelsEnabled: true`。请参阅限制哪些通道插件可以运行 |
...
分类器从用户设置、`.costrict/settings.local.json` 和托管设置中读取 `autoMode`。它不从 `.costrict/settings.json` 中的共享项目设置读取，因为签入的仓库可能会注入自己的 allow 规则。

| 范围                      | 文件                          | 用途                                             |
| :------------------------- | :---------------------------- | :-------------------------------------------------- |
| 单个开发者              | `~/.costrict/settings.json`     | 个人信任的基础设施                     |
| 单个项目、单个开发者 | `.costrict/settings.local.json` | 每个项目的信任存储桶或服务，已 gitignore |
