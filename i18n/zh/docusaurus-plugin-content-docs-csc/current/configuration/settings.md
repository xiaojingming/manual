...
| **用户**    | `~/.costrict/` 目录                                                             | 您，跨所有项目             | 否                     |
| **项目** | 仓库中的 `.costrict/`                                                           | 此仓库的所有协作者 | 是（提交到 git） |
| **本地**   | `.costrict/settings.local.json`                                                      | 您，仅在此仓库中         | 否（gitignored）        |
...
| 功能         | 用户位置             | 项目位置                   | 本地位置                 |
| :-------------- | :------------------------ | :--------------------------------- | :----------------------------- |
| **设置**    | `~/.costrict/settings.json` | `.costrict/settings.json`            | `.costrict/settings.local.json`  |
| **子代理**   | `~/.costrict/agents/`       | `.costrict/agents/`                  | 无                           |
| **MCP 服务器** | `~/.claude.json`          | `.mcp.json`                        | `~/.claude.json`（按项目） |
| **插件**     | `~/.costrict/settings.json` | `.costrict/settings.json`            | `.costrict/settings.local.json`  |
| **AGENTS.md**   | `~/.costrict/AGENTS.md`     | `AGENTS.md` 或 `.costrict/AGENTS.md` | `AGENTS.local.md`              |
...
* **用户设置**定义在 `~/.costrict/settings.json` 中，适用于所有项目。
* **项目设置**保存在您的项目目录中：
  * `.costrict/settings.json` 用于签入源代码控制并与团队共享的设置
  * `.costrict/settings.local.json` 用于未签入的设置，适用于个人偏好和实验。CSC 在创建时会配置 git 忽略 `.costrict/settings.local.json`。
* **托管设置**：对于需要集中控制的组织，CSC 支持多种托管设置的交付机制。所有机制使用相同的 JSON 格式，且不能被用户或项目设置覆盖：

  * **服务器管理的设置**：通过 costrict.ai 管理控制台从 CoStrict 的服务器交付。参见服务器管理的设置。
  * **MDM/OS 级别策略**：通过 macOS 和 Windows 上的本机设备管理交付：
    * macOS：`com.anthropic.claudecode` 托管偏好域（通过 Jamf、Iru (Kandji) 或其他 MDM 工具中的配置文件部署）
    * Windows：`HKLM\SOFTWARE\Policies\CoStrict` 注册表键，包含 JSON 的 `Settings` 值（REG\_SZ 或 REG\_EXPAND\_SZ）（通过组策略或 Intune 部署）
    * Windows（用户级别）：`HKCU\SOFTWARE\Policies\CoStrict`（最低策略优先级，仅在不存在管理员级别来源时使用）
  * **基于文件**：部署到系统目录的 `managed-settings.json` 和 `managed-mcp.json`：
    ...
    * Windows：`C:\Program Files\CoStrict\`
    ...
    > **⚠️ 警告：** 自 v2.1.75 起，旧版 Windows 路径 `C:\ProgramData\CoStrict\managed-settings.json` 不再受支持。部署设置到该位置的管理员必须将文件迁移到 `C:\Program Files\CoStrict\managed-settings.json`。
...
| `allowedChannelPlugins`           | （仅托管设置）允许推送消息的通道插件白名单。设置后替换默认的 CoStrict 白名单。未定义 = 回退到默认值，空数组 = 阻止所有通道插件。需要 `channelsEnabled: true`。参见限制可运行的通道插件                                                                                                                                                                                                       | `[{ "marketplace": "claude-plugins-official", "plugin": "telegram" }]`                                                         |
...
| `attribution`                     | 自定义 git 提交和拉取请求的归属。参见归属设置                                                                                                                                                                                                                                                                                                                                                                                                                                               | `{"commit": "🤖 Generated with CSC", "pr": ""}`                                                                        |
| `autoMemoryDirectory`             | 自动记忆存储的自定义目录。接受 `~/` 展开的路径。在项目设置（`.costrict/settings.json`）中不接受，以防止共享仓库将记忆写入重定向到敏感位置。从策略、本地和用户设置中接受                                                                                                                                                                                                                                                                                                                                                                                           | `"~/my-memory-dir"`                                                                                                            |
...
| `fileSuggestion`                  | 为 `@` 文件自动补全配置自定义脚本。参见文件建议设置                                                                                                                                                                                                                                                                                                                                                                                                                                           | `{"type": "command", "command": "~/.costrict/file-suggestion.sh"}`                                                               |
...
| `includeCoAuthoredBy`             | **已弃用**：改用 `attribution`。是否在 git 提交和拉取请求中包含 `co-authored-by CSC` 署名（默认：`true`）                                                                                                                                                                                                                                                                                                                                                                                                      | `false`                                                                                                                        |
...
| `language`                        | 配置 CSC  的首选响应语言（例如 `"japanese"`、`"spanish"`、`"french"`）。CSC 将默认以此语言响应。还设置语音听写语言                                                                                                                                                                                                                                                                                                           | `"japanese"`                                                                                                                   |
| `model`                           | 覆盖 CSC 使用的默认模型                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `"claude-sonnet-4-6"`                                                                                                          |
| `modelOverrides`                  | 将 CoStrict 模型 ID 映射到提供商特定的模型 ID，如 Bedrock 推理配置文件 ARN。每个模型选择器条目在调用提供商 API 时使用其映射值。参见按版本覆盖模型 ID                                                                                                                                                                                                                                                                                        | `{"claude-opus-4-6": "arn:aws:bedrock:..."}`                                                                                   |
...
| `plansDirectory`                  | 自定义计划文件的存储位置。路径相对于项目根目录。默认：`~/.costrict/plans`                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `"./plans"`                                                                                                                    |
...
| `statusLine`                      | 配置自定义状态行以显示上下文。参见 `statusLine` 文档                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `{"type": "command", "command": "~/.costrict/statusline.sh"}`                                                                    |
...
| `additionalDirectories`             | 文件访问的附加工作目录。大多数 `.costrict/` 配置不会从这些目录中发现                                            | `[ "../docs/" ]`                                                       |
...
| `skipDangerousModePermissionPrompt` | 跳过通过 `--dangerously-skip-permissions` 或 `defaultMode: "bypassPermissions"` 进入绕过权限模式前显示的确认提示。在项目设置（`.costrict/settings.json`）中设置时被忽略，以防止不受信任的仓库自动绕过提示 | `true`                                                                 |
...
| `network.httpProxyPort`                | 如果您希望自带代理使用的 HTTP 代理端口。如果未指定，CSC 将运行自己的代理。                                                                                                                                                                                                                                      | `8080`                          |
| `network.socksProxyPort`               | 如果您希望自带代理使用的 SOCKS5 代理端口。如果未指定，CSC 将运行自己的代理。                                                                                                                                                                                                                                    | `8081`                          |
...
| `./` 或无前缀 | 相对于项目根目录（对于项目设置）或 `~/.costrict`（对于用户设置） | `.costrict/settings.json` 中的 `./output` 解析为 `<project-root>/output` |
...
* **`sandbox.filesystem` 设置**（如上所示）：在 OS 级别沙箱边界控制路径。这些限制适用于所有子进程命令（例如 `kubectl`、`terraform`、`npm`），而不仅仅是 CSC 的文件工具。
* **权限规则**：使用 `Edit` 允许/拒绝规则控制 CSC 的文件工具访问，使用 `Read` 拒绝规则阻止读取，使用 `WebFetch` 允许/拒绝规则控制网络域。这些规则中的路径也合并到沙箱配置中。
...
```text
🤖 Generated with [CSC](https://costrict.ai/claude-code)
...
3. **本地项目设置**（`.costrict/settings.local.json`）
   * 个人的项目特定设置

4. **共享项目设置**（`.costrict/settings.json`）
   * 源代码控制中的团队共享项目设置

5. **用户设置**（`~/.costrict/settings.json`）
   * 个人全局设置
...
* **记忆文件（`AGENTS.md`）**：包含 CSC 在启动时加载的指令和上下文
* **设置文件（JSON）**：配置权限、环境变量和工具行为
* **技能**：可以使用 `/skill-name` 调用或由 CSC 自动加载的自定义提示
...
CSC 的内部系统提示未发布。要添加自定义指令，请使用 `AGENTS.md` 文件或 `--append-system-prompt` 标志。

### 排除敏感文件

要阻止 CSC 访问包含敏感信息（如 API 密钥、机密和环境文件）的文件，请在 `.costrict/settings.json` 文件中使用 `permissions.deny` 设置：
...
* **用户子代理**：`~/.costrict/agents/` - 跨所有项目可用
* **项目子代理**：`.costrict/agents/` - 特定于您的项目，可与团队共享
...
* **用户设置**（`~/.costrict/settings.json`）：个人插件偏好
* **项目设置**（`.costrict/settings.json`）：与团队共享的项目特定插件
* **本地设置**（`.costrict/settings.local.json`）：按机器覆盖（不提交）
