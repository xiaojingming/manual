...
> CSC 从这里读取 AGENTS.md、settings.json、hooks、skills、commands、subagents、rules 和自动记忆。探索你项目中的 .costrict 目录和主目录中的 ~/.costrict。

CSC 从你的项目目录和主目录中的 `~/.costrict` 读取指令、设置、Skills、Subagents和记忆。将项目文件提交到 git 以便与团队共享；`~/.costrict` 中的文件是适用于你所有项目的个人配置。

如果你设置了 `CLAUDE_CONFIG_DIR`，此页面上的每个 `~/.costrict` 路径都将位于该目录下。

大多数用户只需编辑 `AGENTS.md` 和 `settings.json`。目录的其余部分是可选的：根据需要添加Skills、规则或Subagents。
...
- AGENTS.md 会加载到每个会话中。如果某些内容只与特定任务相关，请将其移到 skill 或路径范围的 rule 中，这样只会在需要时加载
- 列出你最常运行的命令，如构建、测试和格式化，这样 CSC 无需你每次都拼写出来
- 运行 `/memory` 可以在会话中打开和编辑 AGENTS.md
- 也可以放在 `.costrict/AGENTS.md`，如果你希望保持项目根目录整洁
...
**何时加载：** 覆盖全局 `~/.costrict/settings.json`。本地设置、CLI 标志和托管设置覆盖此项

**描述：** CSC 直接应用的设置。权限控制 CSC 可以使用哪些命令和工具；hooks 在会话的特定点运行你的脚本。与 CSC 作为指导读取的 AGENTS.md 不同，这些设置无论 CSC 是否遵循都会被强制执行。
...
**描述：** 将项目指令拆分为主题文件，可以根据文件路径有条件地加载。没有 `paths:` frontmatter 的规则在会话开始时加载，就像 AGENTS.md；有 `paths:` 的规则只在 CSC 读取匹配文件时加载。

与 AGENTS.md 一样，规则是 CSC 读取的指导，而不是 CSC 强制执行的配置。要保证行为，请使用 hooks 或 permissions。

**提示：**
- 使用带 glob 的 `paths:` frontmatter 将规则限定到目录或文件类型
- 子目录有效：`.costrict/rules/frontend/react.md` 会被自动发现
- 当 AGENTS.md 接近 200 行时，开始拆分为规则
...
**描述：** 输出风格通常是个人化的，所以大多数放在 `~/.costrict/output-styles/`。如果你的团队共享一种风格（如每个人都使用的审查模式），请放在这里。完整说明和示例请参见全局选项卡。
...
**描述：** 在 frontmatter 中设置了 `memory: project` 的Subagents会在此获得一个专用记忆目录。这与你在 `~/.costrict/projects/` 的主会话自动记忆不同：每个Subagents读写自己的 MEMORY.md，而不是你的。

**提示：**
- 仅为设置了 `memory:` frontmatter 字段的Subagents创建
- 此目录保存项目范围的Subagents记忆，旨在与团队共享。要将记忆排除在版本控制之外，使用 `memory: local`，它会写入 `.costrict/agent-memory-local/`。对于跨项目记忆，使用 `memory: user`，它会写入 `~/.costrict/agent-memory/`
- 主会话自动记忆是不同的功能；参见全局选项卡中的 `~/.costrict/projects/`
...
- `projects` 键跟踪每个项目的状态，如信任对话框接受和上次会话指标。你在会话中批准的权限规则会写入 `.costrict/settings.local.json`
...
你的个人配置，适用于所有项目。这是项目 .costrict/ 目录的全局对应物。此处的文件适用于你工作的每个项目，永远不会提交到任何仓库。
...
**描述：** 你的全局指令文件。在会话开始时与项目 AGENTS.md 一起加载，因此两者同时在上下文中。当指令冲突时，项目级指令优先。将此文件限于适用于所有地方的偏好：响应风格、提交格式、个人约定。

**提示：**
- 保持简短，因为它会与项目自己的 AGENTS.md 一起加载到每个项目的上下文中
- 适用于响应风格、提交格式和个人约定
...
设置遵循优先级顺序：项目 `settings.json` 覆盖你在此处设置的任何匹配键。这与 AGENTS.md 不同，全局和项目文件都加载到上下文中，而不是按键合并。
...
**描述：** 与项目 .costrict/rules/ 相同，但适用于所有地方。将你想在所有工作中使用的约定放在这里，如个人代码风格或提交消息格式。
...
**描述：** 在 frontmatter 中设置了 `memory: user` 的Subagents在此存储跨所有项目持久化的知识。对于项目范围的Subagents记忆，请参见 `.costrict/agent-memory/`。
...
| `AGENTS.local.md` | 项目根目录 | 你在此项目的私人偏好，与 AGENTS.md 一起加载。手动创建并添加到 `.gitignore` |
| 已安装的Plugins | `~/.costrict/plugins/` | 克隆的市场、已安装的Plugins版本和每个Plugins的数据，由 `csc plugin` 命令管理。孤立版本在Plugins更新或卸载后 7 天删除。参见Plugins缓存 |

`~/.costrict` 还保存 CSC 在工作时写入的数据：转录、提示历史、文件快照、缓存和日志。参见下面的应用数据。

## 文件参考

此表列出了页面涵盖的每个文件。项目范围文件位于你仓库的 `.costrict/` 下（或对于 `AGENTS.md`、`.mcp.json` 和 `.worktreeinclude` 在根目录）。全局范围文件位于 `~/.costrict/` 并适用于所有项目。
...
| `/memory` | 哪些 AGENTS.md 和 rules 文件已加载，以及自动记忆条目 |
...
除了你编写的配置外，`~/.costrict` 还保存 CSC 在会话期间写入的数据。这些文件是纯文本。任何通过工具的内容都会写入磁盘上的转录：文件内容、命令输出、粘贴的文本。
...
| 删除 | 你会丢失 |
| --- | --- |
| `~/.costrict/projects/` | 过去会话的恢复、继续和回退 |
| `~/.costrict/history.jsonl` | 上箭头提示回忆 |
| `~/.costrict/file-history/` | 过去会话的检查点恢复 |
| `~/.costrict/stats-cache.json` | `/cost` 显示的历史总计 |
| `~/.costrict/backups/` | 过去配置迁移中 `~/.claude.json` 的回滚副本 |
| `~/.costrict/debug/`、`~/.costrict/plans/`、`~/.costrict/paste-cache/`、`~/.costrict/image-cache/`、`~/.costrict/session-env/` | 无用户可见内容 |
| `~/.costrict/todos/` | 无。旧版目录，当前版本不再写入。 |

不要删除 `~/.claude.json`、`~/.costrict/settings.json` 或 `~/.costrict/plugins/`：它们保存你的认证、偏好和已安装的Plugins。
