...
> **💡 提示：**
>
> * 在 `.costrict/agents/` 中创建项目特定的子代理以便团队共享
...
>
> * 文件路径可以是相对路径或绝对路径
> * @ 文件引用会将文件目录和父目录中的 `AGENTS.md` 添加到上下文中
> * 目录引用显示文件列表，而不是内容
...
# 创建 .costrict/worktrees/feature-auth/ 和一个新分支
...
Worktree 创建在 `<repo>/.costrict/worktrees/<name>`，并从默认远程分支（即 `origin/HEAD` 指向的位置）分出。Worktree 分支命名为 `worktree-<name>`。
...
> **💡 提示：** 将 `.costrict/worktrees/` 添加到你的 `.gitignore` 中，以防止 worktree 内容作为未跟踪文件出现在你的主仓库中。
...
1. **将 hook 添加到你的设置中**
    ...
    打开 `~/.costrict/settings.json` 并添加一个调用你平台原生通知命令的 `Notification` hook：
...
| 选项                                                 | 运行位置                     | 最适合                                                                                                      |
| :----------------------------------------------------- | :-------------------------------- | :------------------------------------------------------------------------------------------------------------ |
| 云端计划任务       | CoStrict 管理的基础设施  | 即使你的计算机关闭也应该运行的任务。在 costrict.ai/code 配置。  |
