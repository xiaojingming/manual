...
> **💡 提示：** 运行 `/init` 根据当前项目结构生成一个 AGENTS.md 起始文件，然后随时间逐步完善。

AGENTS.md 是 CSC 在每次对话开始时读取的特殊文件。包含 Bash 命令、代码风格和工作流规则。这为 CSC 提供了仅从代码中无法推断的持久上下文。

`/init` 命令分析你的代码库以检测构建系统、测试框架和代码模式，为你提供完善的基础。

AGENTS.md 文件没有必需的格式，但保持简短且人类可读。例如：
...
AGENTS.md 在每次会话中都会加载，所以只包含广泛适用的内容。对于仅有时相关的领域知识或工作流，请改用Skills。CSC 按需加载它们，不会使每次对话膨胀。

保持简洁。对于每一行，问自己：*"删除这行会导致 CSC 犯错吗？"* 如果不会，就删掉它。臃肿的 AGENTS.md 文件会导致 CSC 忽略你的实际指令！
...
如果 CSC 尽管有规则仍然不断做你不想做的事，文件可能太长了，规则被淹没了。如果 CSC 问你的问题在 AGENTS.md 中有答案，措辞可能有歧义。像对待代码一样对待 AGENTS.md：出问题时审查它，定期修剪，通过观察 CSC 的行为是否真的改变来测试修改。

你可以通过添加强调（例如 "IMPORTANT" 或 "YOU MUST"）来调整指令以提高遵守度。将 AGENTS.md 提交到 git，这样你的团队可以贡献。这个文件的价值会随时间复利增长。

AGENTS.md 文件可以使用 `@path/to/import` 语法导入其他文件：
...
你可以在多个位置放置 AGENTS.md 文件：

* **主文件夹 (`~/.costrict/AGENTS.md`)**：适用于所有 CSC 会话
* **项目根目录 (`./AGENTS.md`)**：提交到 git 与团队共享
* **项目根目录 (`./AGENTS.local.md`)**：个人项目特定备注；将此文件添加到 `.gitignore` 以便不与团队共享
* **父目录**：适用于 monorepo，`root/AGENTS.md` 和 `root/foo/AGENTS.md` 都会自动加载
* **子目录**：CSC 在处理这些目录中的文件时按需加载子目录的 AGENTS.md 文件
...
Hooks在 CSC 工作流的特定点自动运行脚本。与 AGENTS.md 指令（是建议性的）不同，Hooks是确定性的，保证操作会执行。

CSC 可以为你编写Hooks。尝试这样的提示：*"编写一个在每次文件编辑后运行 eslint 的Hooks"* 或 *"编写一个阻止写入 migrations 文件夹的Hooks。"* 直接编辑 `.costrict/settings.json` 来手动配置Hooks，运行 `/hooks` 浏览已配置的内容。

### 创建Skills

> **💡 提示：** 在 `.costrict/skills/` 中创建 `SKILL.md` 文件，为 CSC 提供领域知识和可重用的工作流。

Skills扩展了 CSC 的知识，添加了特定于你的项目、团队或领域的信息。CSC 在相关时自动应用它们，或者你可以用 `/skill-name` 直接调用。

通过在 `.costrict/skills/` 中添加包含 `SKILL.md` 的目录来创建Skills：
...
> **💡 提示：** 在 `.costrict/agents/` 中定义专门的助手，CSC 可以将隔离任务委托给它们。
...
Plugins将Skills、Hooks、Subagents和 MCP 服务器打包为来自社区和 CoStrict 的单一可安装单元。如果你使用类型化语言，安装代码智能Plugins，为 CSC 提供精确的符号导航和编辑后的自动错误检测。
...
* 在 AGENTS.md 中自定义压缩行为，使用如 `"When compacting, always preserve the full list of modified files and any test commands"` 的指令，确保关键上下文在摘要后保留
...
* Web 上的 CSC：在 CoStrict 的安全云基础设施上的隔离 VM 中运行。
...
* **过度指定的 AGENTS.md。** 如果你的 AGENTS.md 太长，CSC 会忽略其中一半，因为重要规则被噪音淹没。
