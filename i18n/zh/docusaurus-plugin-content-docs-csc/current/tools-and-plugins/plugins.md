...
| 方式                                                    | Skills名称          | 最适合                                                                                        |
| :---------------------------------------------------------- | :------------------- | :---------------------------------------------------------------------------------------------- |
| **独立配置**（`.costrict/` 目录）                       | `/hello`             | 个人工作流、项目特定自定义、快速实验                          |
...
> **💡 提示：** 先在 `.costrict/` 中使用独立配置进行快速迭代，当你准备好共享时再转换为Plugins。
...
你会看到 CSC 用问候语回应。运行 `/help` 可以看到你的Skills列在Plugins命名空间下。
...
CSC 将按名字问候你。有关向Skills传递参数的更多信息，请参阅Skills。
...
Plugins可以包含代理Skills来扩展 CSC 的能力。Skills是模型调用的：CSC 根据任务上下文自动使用它们。
...
LSP（语言服务器协议）Plugins为 CSC 提供实时代码智能。如果你需要支持没有官方 LSP Plugins的语言，可以通过在Plugins中添加 `.lsp.json` 文件来创建自己的 LSP Plugins：
...
要向官方 CoStrict 市场提交Plugins，请使用以下应用内提交表单之一：

* **costrict.ai**：costrict.ai/settings/plugins/submit
* **Console**：platform.costrict.ai/plugins/submit

> **注意：** 有关完整的技术规范、调试技术和分发策略，请参阅Plugins参考。

## 将现有配置转换为Plugins

如果你已经在 `.costrict/` 目录中有Skills或Hooks，可以将它们转换为Plugins以便于共享和分发。
...
创建 `my-plugin/hooks/hooks.json` 并配置你的Hooks。从 `.costrict/settings.json` 或 `settings.local.json` 复制 `hooks` 对象，因为格式是相同的。命令通过 stdin 接收 JSON 格式的Hooks输入，因此使用 `jq` 提取文件路径：
...
| 独立配置（`.costrict/`）       | Plugins                           |
| :---------------------------- | :------------------------------- |
| 仅在一个项目中可用 | 可以通过市场共享   |
| 文件在 `.costrict/commands/`  | 文件在 `plugin-name/commands/` |
| Hooks在 `settings.json`      | Hooks在 `hooks/hooks.json`      |
| 必须手动复制以共享   | 使用 `/plugin install` 安装   |

> **注意：** 迁移后，你可以从 `.costrict/` 中删除原始文件以避免重复。Plugins版本在加载时将优先。
