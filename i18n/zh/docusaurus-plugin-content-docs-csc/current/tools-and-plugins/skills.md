...
当你不断将相同的操作手册、检查清单或多步骤流程粘贴到聊天中，或者当 AGENTS.md 的某个部分已经从事实描述变成了操作流程时，就该创建一个Skills了。与 AGENTS.md 的内容不同，Skills的主体只在被使用时才加载，因此长篇参考材料在你需要之前几乎不消耗任何资源。

> **注意：** 对于内置命令（如 `/help` 和 `/compact`）以及内置Skills（如 `/debug` 和 `/simplify`），请参阅命令参考。
>
> **自定义命令已合并到Skills中。** `.costrict/commands/deploy.md` 处的文件和 `.costrict/skills/deploy/SKILL.md` 处的Skills都会创建 `/deploy` 并且工作方式相同。你现有的 `.costrict/commands/` 文件继续有效。Skills增加了可选功能：用于存放支持文件的目录、用于控制由你还是 CSC 调用的 frontmatter，以及 CSC 在相关时自动加载的能力。
...
创建 `~/.costrict/skills/explain-code/SKILL.md`：
...
当不同级别的Skills同名时，高优先级位置优先：企业级 > 个人 > 项目。PluginsSkills使用 `plugin-name:skill-name` 命名空间，因此不会与其他级别冲突。如果你在 `.costrict/commands/` 中有文件，它们的工作方式相同，但如果Skills和命令同名，Skills优先。

#### 从嵌套目录自动发现

当你在子目录中处理文件时，CSC 会自动从嵌套的 `.costrict/skills/` 目录中发现Skills。例如，如果你在 `packages/frontend/` 中编辑文件，CSC 也会在 `packages/frontend/.costrict/skills/` 中查找Skills。这支持了各包拥有自己Skills的 monorepo 设置。
...
> **注意：** `.costrict/commands/` 中的文件仍然有效，并支持相同的 frontmatter。推荐使用Skills，因为它们支持支持文件等额外功能。

#### 来自额外目录的Skills

`--add-dir` 标志授予文件访问权限而非配置发现，但Skills是一个例外：额外目录中的 `.costrict/skills/` 会被自动加载并被实时变更检测捕获，因此你可以在会话期间编辑这些Skills而无需重启。

其他 `.costrict/` 配置（如Subagents、命令和输出样式）不会从额外目录加载。有关加载和不加载内容的完整列表以及跨项目共享配置的推荐方式，请参阅异常表。

> **注意：** 来自 `--add-dir` 目录的 AGENTS.md 文件默认不加载。要加载它们，请设置 `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1`。请参阅从额外目录加载。
...
| 方式                           | 系统提示                               | 任务                   | 还会加载                       |
| :----------------------------- | :------------------------------------- | :--------------------- | :----------------------------- |
| 带 `context: fork` 的Skills      | 来自代理类型（`Explore`、`Plan` 等）   | SKILL.md 内容          | AGENTS.md                      |
| 带 `skills` 字段的Subagents       | Subagents的 markdown 主体                 | CSC 的委派消息         | 预加载的Skills + AGENTS.md       |
...
`agent` 字段指定使用哪个Subagents配置。选项包括内置代理（`Explore`、`Plan`、`general-purpose`）或 `.costrict/agents/` 中的任何自定义Subagents。如果省略，使用 `general-purpose`。
...
* **项目Skills**：将 `.costrict/skills/` 提交到版本控制
...
创建 `~/.costrict/skills/codebase-visualizer/SKILL.md`。描述告诉 CSC 何时激活此Skills，指令告诉 CSC 运行捆绑的脚本：
...
创建 `~/.costrict/skills/codebase-visualizer/scripts/visualize.py`。此脚本扫描目录树并生成一个自包含的 HTML 文件，包含：
