...
1.1.2 您可以使用第三方账号登录并使用CoStrict ，经过您的同意，我们将获取您在第三方平台注册的公开信息（包括账户名、头像、昵称以及您授权的其他信息），用于与 CoStrict 账号绑定，使您可以直接登录并使用本产品和相关服务。当前CoStrict仅支持通过Github账号进行登录，基于前述目的，我们会记录您授权同步的**Github用户名及昵称信息、ID、头像、账户绑定的邮箱地址信息**。

**1.2 使用CoStrict的过程中收集的信息**

1.2.1 一般而言，CoStrict服务需要预先充值后才能使用，用户须在登录账户后方可充值。在充值的过程中，我们仅会通过日志记录页面响应情况、以及是否充值成功的结果信息，不涉及收集用户的其他个人信息。充值完成后，用户可要求开具发票，用户选择发票抬头为个人时，需提供**邮箱地址、手机号码**，用于接收电子发票，用户选择发票抬头为企业时，则须准确提供发票抬头信息，同时填写**邮箱地址**用于接收电子发票。

1.2.2 当您与CoStrict AI编程助手沟通时，我们会自动接收您**输⼊的数据（您输⼊的指令信息、代码⽚段）**为您提供代码生成、代码补全、代码问答等功能。除了您主动输入的数据以外，为了按照您的指令执行相关任务，我们可能还需要**读取您设置的AI编程助手工作区内的数据（您用于打开AI编程助手应用的文件夹将默认为应用的工作区），以及可能需要在工作区中写入数据；工作区以外的数据读取也将严格按照用户的权限配置执行，在用户未明确操作允许的情况下，将不会读取工作区以外的任何数据**。由于AI编程助手的能力需调用大模型提供支持，这些模型中既包含部分我们已本地化部署的模型，也包含我们通过接口调用或其他方式接入的外部第三方模型，其中可能有部分模型部署运行在中国境外，因此本条前述说明的数据可能会在您使用过程中传输至外部第三方的服务器、甚至中国境外进行处理；用户也可选择对接自身本地已部署的模型，此种情况下前述说明的数据仍需传输至我们的服务器，但不会传输给外部第三方。**请您知悉并理解：由于您的个人信息涉及您的隐私权益，因此请您自行确保您主动输入的数据和允许我们读取的数据中不包含您的相关个人信息，以避免我们被动接收和处理您的个人信息、甚至按照您的需求和选择将您的个人信息传输给其他第三方；针对您的其他数据，也请您在选择模型时事先评估确认是否同意将数据提供给第三方、甚至传输至境外进行处理；如您需确认我们的应用与支持选择的模型之间的对接方式等情况，您可通过本政策提供的联系方式与我们联系。**

除本隐私保护政策所列使用场景及目的以及履行法定义务外，我们不会将您上传和生成的代码⽂件用于其他任何⾮经您授权的场景。
...
2.5 如前述第1.2.2条说明，由于用户使用CoStrict提供的服务的过程中，可自主选择用于提供技术支持的大模型，根据用户的选择，用户使用过程中相关数据可能需传输至第三方模型部署的服务器上进行处理，这部分数据将存储在第三方服务器所在地、以及按照第三方服务提供方与用户之间的相关约定进行处理，针对这部分数据，深信服将仅能依据服务对接的情况、在用户提出请求时为用户提供相关信息，但无法承诺按照本隐私保护政策保护您的数据权益，因此请您在选择服务时务必事先谨慎进行必要的评估和确认。
...
您可以根据 CoStrict《用户协议》的相关规定申请注销您的账号，境内用户可要求在15个工作日内完成账号注销相关的核查和处理。请您知悉并理解，注销账号的行为是不可逆的行为，当您注销账号后，我们将停止为您提供任何服务，并在超出相应的存储期限后删除有关您账号的⼀切信息或对相关信息进行匿名化处理，因法律法规规定需要留存个⼈信息的，我们承诺将其单独存储，并不会将该等信息用于日常业务活动中。
